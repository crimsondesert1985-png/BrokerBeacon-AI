...
const workspaceTerms={home:['home','today','priority','dashboard'],prospects:['prospect','watchtower','scout','search','pipeline','company','route','broker','discovery'],outreach:['call','email','text','voice','template','campaign','follow','outreach','activity'],intelligence:['intelligence','production','guideline','report','market','insight','analytics'],settings:['setting','integration','billing','team','access','role','workspace','admin','owner']};
function legacyButtons(){return [...document.querySelectorAll('.bb-legacy-list>button,.bb-legacy-tools button')]}
function buttons(){const legacy=legacyButtons();const others=[...document.querySelectorAll('aside nav button,aside button')].filter(b=>!b.closest('#bb-simple-flow')&&!b.closest('.bb-core-nav')&&!b.closest('.bb-workspace-strip')&&!legacy.includes(b));return [...legacy,...others]}
function clickWorkingButton(terms){const hit=buttons().find(b=>!b.disabled&&terms.some(t=>clean((b.textContent||'')+' '+(b.title||'')+' '+(b.id||'')).includes(t)));if(!hit)return false;hit.click();hit.scrollIntoView({behavior:'smooth',block:'nearest'});return true}
function openChoice(key){const terms=choices[key]||[];if(clickWorkingButton(terms))return;const paths={team:'/workspace/team'};if(paths[key])location.href=paths[key]}
function openWorkspace(id){const terms=workspaceTerms[id]||[];if(clickWorkingButton(terms))return;if(id==='settings')location.href='/workspace/team'}
function repairWorkspaceClicks(){if(document.documentElement.dataset.bbWorkspaceRepair)return;document.documentElement.dataset.bbWorkspaceRepair='1';document.addEventListener('click',ev=>{const control=ev.target.closest('.bb-core-btn,.bb-workspace-strip button');if(!control)return;const id=control.dataset.workspace;if(!id)return;ev.preventDefault();ev.stopImmediatePropagation();document.querySelectorAll('.bb-core-btn,.bb-workspace-strip button').forEach(b=>b.classList.toggle('active',b.dataset.workspace===id));localStorage.setItem('bb-workspace',id);openWorkspace(id)},true)}
...
function simplifyNav(){const nav=document.querySelector('aside nav');if(!nav||nav.dataset.simplified||document.querySelector('.bb-core-nav'))return;nav.dataset.simplified='1';const all=[...nav.children].filter(x=>x.tagName==='BUTTON');const primaryTerms=['home','today','priority','prospect','pipeline','call prep'];const advanced=all.filter(b=>!primaryTerms.some(t=>clean(b.textContent).includes(t))&&!['team & access','ember control tower'].some(t=>clean(b.textContent).includes(t)));if(advanced.length<4)return;const more=document.createElement('details');more.className='bb-nav-more';const summary=document.createElement('summary');summary.textContent='More tools';more.appendChild(summary);advanced.forEach(b=>more.appendChild(b));nav.appendChild(more)}
...
function run(){repairWorkspaceClicks();addFlow();explainButtons();simplifyNav()}if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',run);else run();
})();</script>'''
