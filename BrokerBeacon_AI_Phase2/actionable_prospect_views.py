...
def install_actionable_prospect_views(app, db_path):
    ...
    def prospect_catalog():
        ...
        body = f'''<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>BrokerBeacon Actionable Prospects</title><style>body{{font:14px Inter,Arial;background:#07162e;color:#eef4ff;margin:0}}header,main{{padding:22px}}header{{background:#0d2347}}form{{display:flex;gap:8px;margin-bottom:16px}}input,select,button{{padding:10px;border-radius:8px;border:1px solid #ffffff22;background:#102b52;color:white}}input{{flex:1}}table{{width:100%;border-collapse:collapse;background:#0c2040}}th,td{{padding:11px;border-bottom:1px solid #ffffff18;text-align:left;vertical-align:top}}small{{display:block;color:#9fb2d1;margin-top:4px}}a{{color:#8fd7ff}}</style></head><body><header><strong>BrokerBeacon · Actionable Prospects</strong><div>{len(items)} prospects with public contact information</div></header><main><p>Only records with a public phone or email are shown. Verify licensing and contact details before relying on them.</p><form method="get"><input name="q" value="{html.escape(query)}" placeholder="Search company, city, or NMLS"><select name="state">{''.join(options)}</select><button>Filter</button></form><table><thead><tr><th>Company</th><th>Contact</th><th>Location</th><th>Source</th><th>Score</th><th>Verification</th></tr></thead><tbody>{''.join(rows) if rows else '<tr><td colspan="6">No actionable prospects match this filter yet.</td></tr>'}</tbody></table></main></body></html>'''
