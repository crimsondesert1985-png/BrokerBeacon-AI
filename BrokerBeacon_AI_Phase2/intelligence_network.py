...
SCHEMA = "\n".join([
    ...
    "id integer primary key,node_type text not null,external_key text not null,label text not null,",
    "attributes_json text not null default '{}',confidence integer not null default 50,source text default '',",
    "created_at text not null,updated_at text not null,unique(node_type,external_key));",
    "create table if not exists intelligence_edges(",
    "id integer primary key,from_node_id integer not null,relationship text not null,to_node_id integer not null,",
    "attributes_json text not null default '{}',confidence integer not null default 50,source text default '',",
    "created_at text not null,updated_at text not null,unique(from_node_id,relationship,to_node_id),",
    "foreign key(from_node_id) references intelligence_nodes(id),foreign key(to_node_id) references intelligence_nodes(id));",
    "create table if not exists company_intelligence_profiles(",
    "id integer primary key,company_key text not null unique,company_name text not null,website text default '',",
    "states_json text not null default '[]',products_json text not null default '[]',leadership_json text not null default '[]',",
    "growth_signals_json text not null default '[]',hiring_signals_json text not null default '[]',",
    "technology_signals_json text not null default '[]',public_news_json text not null default '[]',",
    "opportunity_summary text default '',confidence integer not null default 50,last_refreshed_at text default '',",
    "created_at text not null,updated_at text not null);",
    "create table if not exists planner_cycles(",
    "id integer primary key,status text not null default 'Planned',selected_state text default '',",
    "selected_strategy text default '',rationale text default '',actions_json text not null default '[]',",
    "created_at text not null,executed_at text default '');",
    "create table if not exists voice_outcomes(",
    "id integer primary key,prospect_type text default '',prospect_id integer,company_key text default '',",
    "agent_key text not null default 'coach',outcome text not null,disposition text default '',",
    "duration_seconds integer not null default 0,notes text default '',metadata_json text not null default '{}',",
    "approved_for_learning integer not null default 0,created_at text not null);",
...
def upsert_node(conn: sqlite3.Connection, node_type: str, external_key: str, label: str,
    ...
    conn.execute(sql, (node_type, external_key, label, json.dumps(attributes or {}, sort_keys=True, default=str),
...
def link_nodes(conn: sqlite3.Connection, from_id: int, relationship: str, to_id: int,
    ...
    now = NOW()
    sql = (
        "insert into intelligence_edges(from_node_id,relationship,to_node_id,attributes_json,confidence,source,created_at,updated_at) "
        "values(?,?,?,?,?,?,?,?) on conflict(from_node_id,relationship,to_node_id) do update set "
        "attributes_json=excluded.attributes_json,confidence=max(intelligence_edges.confidence,excluded.confidence),"
        "source=excluded.source,updated_at=excluded.updated_at"
    )
    conn.execute(sql, (from_id, relationship, to_id, json.dumps(attributes or {}, sort_keys=True, default=str),
                       min(max(int(confidence), 0), 100), source, now, now))
    row = conn.execute(
        "select id from intelligence_edges where from_node_id=? and relationship=? and to_node_id=?",
...
def sync_discoveries(conn: sqlite3.Connection, limit: int = 5000) -> dict:
    initialize(conn)
    sql = (
        "select d.*,a.opportunity_score,a.confidence ai_confidence,"
        "a.canonical_company_name,a.canonical_person_name "
        "from discovered_contacts d left join ai_contact_insights a on a.discovered_contact_id=d.id "
        "order by d.id desc limit ?"
    )
    rows = conn.execute(sql, (min(max(int(limit), 1), 50000),)).fetchall()
    ...
    for row in rows:
        ...
        company_id = upsert_node(
            conn, "company", company_key, company_name,
            {"state": row["state"], "domain": row["source_domain"], "email": row["public_email"],
             "phone": row["phone"], "opportunity_score": row["opportunity_score"] or 0},
        ...
        if person_name:
            ...
            person_id = upsert_node(
                conn, "person", person_key, person_name,
                {"role": row["role"], "email": row["public_email"], "phone": row["phone"], "state": row["state"]},
                row["ai_confidence"] or 60, row["source_url"],
            )
            people += 1
...
def upsert_company_profile(conn: sqlite3.Connection, company_key: str, company_name: str, **fields) -> int:
    ...
    sql = (
        ...
        "opportunity_summary,confidence,last_refreshed_at,created_at,updated_at) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) "
        "on conflict(company_key) do update set company_name=excluded.company_name,website=excluded.website,"
        "states_json=excluded.states_json,products_json=excluded.products_json,leadership_json=excluded.leadership_json,"
...
def plan_growth(conn: sqlite3.Connection) -> dict:
    ...
    coverage = {row[0]: row[1] for row in conn.execute("select state,count(*) from discovered_contacts group by state")}
    ...
    cur = conn.execute(
        "insert into planner_cycles(status,selected_state,selected_strategy,rationale,actions_json,created_at) "
        "values('Planned',?,?,?,?,?)",
...
def record_voice_outcome(conn: sqlite3.Connection, *, prospect_type: str, prospect_id: int | None,
    ...
    sql = (
        ...
        "duration_seconds,notes,metadata_json,approved_for_learning,created_at) values(?,?,?,?,?,?,?,?,?,?)"
    )
    cur = conn.execute(sql, (
        prospect_type, prospect_id, company_key, outcome[:120], disposition[:120], max(int(duration_seconds), 0),
        notes[:2000], json.dumps(metadata or {}, sort_keys=True, default=str), int(bool(approve_learning)), NOW(),
    ))
    conn.commit()
...
def dashboard(conn: sqlite3.Connection) -> dict:
    ...
    node_counts = {row[0]: row[1] for row in conn.execute(
        "select node_type,count(*) from intelligence_nodes group by node_type")}
    edge_counts = {row[0]: row[1] for row in conn.execute(
        "select relationship,count(*) from intelligence_edges group by relationship")}
