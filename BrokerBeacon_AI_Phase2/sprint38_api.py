...
def install_sprint38_api(app,db_path):
 ...
 with connect() as conn:init_public(conn);init_enrichment(conn);init_ai(conn);init_ember(conn)
 ...
 def companies():
  ...
  with connect() as conn:rows=conn.execute("""select p.company_name,p.state,p.city,p.source_url,p.source_domain,max(p.created_at) discovered_at,count(distinct d.id) contact_count,max(coalesce(a.opportunity_score,0)) top_score,sum(case when d.review_status='Pending review' then 1 else 0 end) pending_count from public_search_results p left join discovered_contacts d on d.search_result_id=p.id left join ai_contact_insights a on a.discovered_contact_id=d.id where p.candidate_type='Company' and trim(p.company_name)<>'' and (?='' or p.state=?) group by p.company_name,p.state,p.city,p.source_url,p.source_domain order by top_score desc,contact_count desc,discovered_at desc limit ?""",(state,state,limit)).fetchall()
 ...
 def company_detail():
  ...
  with connect() as conn:
   company=conn.execute("select company_name,state,city,source_url,source_domain,max(created_at) discovered_at from public_search_results where company_name=? group by company_name,state,city,source_url,source_domain order by discovered_at desc limit 1",(name,)).fetchone()
   contacts=conn.execute("select d.*,coalesce(a.opportunity_score,0) opportunity_score,coalesce(a.next_best_action,'') next_best_action from discovered_contacts d left join ai_contact_insights a on a.discovered_contact_id=d.id where d.company_name=? order by opportunity_score desc,d.id desc",(name,)).fetchall()
  return jsonify(company=dict(company),contacts=[dict(r) for r in contacts]) if company else (jsonify(error='Company not found'),404)
 @bp.get('/api/platform/sprint39/overview')
 @owner_required
 def sprint39_overview():
  with connect() as conn:
   init_ember(conn);since=(datetime.now()-timedelta(hours=24)).isoformat(timespec='seconds')
   ...
   priorities=[dict(r) for r in conn.execute("""select d.id,d.person_name,d.company_name,d.role,d.phone,d.public_email,d.state,d.source_url,d.review_status,coalesce(a.opportunity_score,0) opportunity_score,coalesce(a.next_best_action,'Review and verify this public contact.') next_best_action,coalesce(a.reasons_json,'[]') reasons_json from discovered_contacts d left join ai_contact_insights a on a.discovered_contact_id=d.id where d.review_status='Pending review' order by opportunity_score desc,d.confidence desc,d.id desc limit 10""")]
