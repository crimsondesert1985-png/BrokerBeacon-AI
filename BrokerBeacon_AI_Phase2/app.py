...
app = Flask(__name__)
BUILD_VERSION = "9.0.1"
BUILD_NAME = "ASH UNDERWRITER PLAIN-ENGLISH ANSWERS"
...
function buildUnderwriterAnswer(d,q){
  const answer=$('#underwriterAnswer'),groups={};
  (d.results||[]).forEach(r=>(groups[r.program]||(groups[r.program]=[])).push(r));
  const names=Object.keys(groups),topics=scenarioTopics(q),pa=d.plain_answer||{};
  const verdict=pa.verdict||'The indexed sources do not support a clear answer yet.';
  const explanation=pa.explanation||'Ash found related source material, but not enough controlling language to give a reliable plain-English conclusion.';
  const basis=pa.basis||((d.results||[])[0]?`Strongest source: ${(d.results||[])[0].program}${(d.results||[])[0].section?' · '+(d.results||[])[0].section:''}.`:'Try a shorter topic or select one agency.');
  const confidence=pa.confidence||'Evidence limited';
  let findings=names.slice(0,5).map(name=>{let r=groups[name][0];return `<div class="finding-card"><b>${esc(name)}${r.section?' · '+esc(r.section):''}</b><p>${esc(conciseGuideText(r.excerpt,190))}</p></div>`}).join('');
  answer.innerHTML=`<div class="answer-head"><div><span class="answer-badge">Ash Underwriter answer</span><div class="answer-title">Plain-English conclusion</div></div><span class="guide-chip">${d.results.length} source match${d.results.length===1?'':'es'}</span></div><div class="plain-answer-box"><div class="plain-answer-label">What Ash thinks <span class="answer-confidence">${esc(confidence)}</span></div><div class="plain-answer-verdict">${esc(verdict)}</div><p class="plain-answer-body">${esc(explanation)}</p><div class="plain-answer-basis">${esc(basis)}</div></div>${guideMode==='scenario'&&topics.length?`<div class="scenario-issues">${topics.map(x=>`<span class="scenario-issue">${esc(x)}</span>`).join('')}</div>`:''}${findings?`<div class="guide-section-heading"><div><div class="kicker">WHY ASH REACHED THAT CONCLUSION</div><h3>Key evidence by agency</h3></div></div><div class="answer-findings">${findings}</div>`:''}<div class="answer-caveat"><b>Decision checkpoint:</b> Ash’s answer is an evidence-based interpretation of the retrieved guide language, not an approval. Confirm the cited source’s effective date, AUS findings, UHM overlays, and underwriting interpretation.</div>`;
...
$$('[data-guide-mode]').forEach(b=>b.onclick=()=>{guideMode=b.dataset.guideMode;$$('[data-guide-mode]').forEach(x=>x.classList.toggle('active',x===b));$('#guideModeHint').textContent=guideMode==='scenario'?'Paste the full borrower, property, income, asset, and transaction scenario.':'Ask a natural-language guideline question.';$('#guideSearch').placeholder=guideMode==='scenario'?'Example: 680 FICO, duplex investment, 15% down, gift funds, self-employed…':'Example: Can gift funds be used on a 2-unit investment property?'});
...
def _fallback_plain_answer(query, results):
    ...
    if not top:
        return {'verdict':'I do not have enough indexed guidance to answer that yet.','explanation':'No sufficiently close official section was retrieved. Narrow the topic or select one agency.','basis':'No controlling source match was found.','confidence':'Insufficient evidence'}
    program=top.get('program','the selected program')
    section=top.get('section','')
    title=top.get('title','the retrieved section')
    text=' '.join((r.get('excerpt') or '') for r in results[:5]).lower()
    # High-value deterministic interpretation for common yes/no phrasing.
    if 'gift' in q and 'investment' in q and ('fannie' in q or program=='Fannie Mae'):
        return {'verdict':'No — not for a Fannie Mae investment-property transaction.','explanation':'The indexed Fannie Mae guidance limits personal gift funds to eligible principal-residence and second-home transactions. An investment property falls outside those eligible occupancy types, so gift funds should not be used to satisfy the required funds for this scenario.','basis':f"Based primarily on {section or title} ({title}). Review the full section for donor, documentation, and minimum-contribution requirements.",'confidence':'High from indexed source'}
    ...
    if re.search(r'\b(can|may|is|are|does|do)\b',q):
        if negative and not positive:
            verdict='Probably no, based on the retrieved language.'
        elif positive and not negative:
            verdict='Probably yes, subject to the listed conditions.'
        else:
            verdict='It appears conditionally possible, but the retrieved language does not support a clean yes-or-no answer.'
    else:
        verdict=f'The most relevant guidance is {program} {section or title}.'
    excerpt=re.sub(r'</?mark>','',top.get('excerpt') or '',flags=re.I)
    excerpt=re.sub(r'\s+',' ',excerpt).strip()
    if len(excerpt)>360: excerpt=excerpt[:357].rsplit(' ',1)[0]+'…'
    return {'verdict':verdict,'explanation':excerpt or 'Open the cited source below to review the controlling language and exceptions.','basis':f"Primary source: {program}{' · '+section if section else ''} · {title}.",'confidence':'Evidence summary'}
...
def _plain_english_guideline_answer(query, results):
    ...
    for i,r in enumerate(results[:8],1):
        ...
        evidence.append(f"SOURCE {i}: {r.get('program')} | {r.get('section') or 'no section'} | {r.get('title')}\n{excerpt[:900]}")
    prompt=("You are Ash Underwriter inside a mortgage account-executive tool. Answer the user's question in plain English using ONLY the supplied official-guide excerpts. "
            "Do not invent requirements, numbers, exceptions, or approvals. Start with a direct conclusion a person would say. If evidence is incomplete, say that clearly. "
            "Return strict JSON with keys verdict, explanation, basis, confidence. verdict: one short sentence. explanation: 2-4 plain-English sentences. "
            "basis: identify the most important agency section(s). confidence must be one of High, Moderate, Limited.\n\nQUESTION:\n"+query+"\n\n"+'\n\n'.join(evidence))
    try:
        body=json.dumps({'model':os.getenv('OPENAI_TEXT_MODEL','gpt-4.1-mini'),'input':prompt,'max_output_tokens':350}).encode()
        ...
        if all(parsed.get(k) for k in ['verdict','explanation','basis']):
            parsed['confidence']=parsed.get('confidence','Moderate')
            return parsed
    except Exception as exc:
        app.logger.warning('Plain-English guideline synthesis fallback: %s',exc)
    return fallback
