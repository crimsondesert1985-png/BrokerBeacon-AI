...
function clearContactForm(){['contactId','contactName','contactRole','contactEmail','contactPhone','contactMobile','contactLinkedin','contactSource','contactVerified','contactNotes'].forEach(id=>$('#'+id).value='');$('#contactPrimary').checked=false;$('#contactDecision').checked=false}
...
load();dash();outreach();followups();dailyPlan();ints();
</script></body></html>'''
...
def init():
    with db() as c:
        ...
        for definition in [
            ...
            "first_contact_date text default ''", "last_call_at text default ''", "last_email_at text default ''",
...
def health():
    try:
        ...
        return jsonify(status="ok", prospects=prospect_count, version="5.2")
...
def save_contact(pid):
    ...
    with db() as c:
        ...
        values=(d.get('name','Company Contact Desk'),d.get('role',''),d.get('email',''),d.get('phone',''),d.get('mobile',''),d.get('nmls',''),d.get('specialties',''),d.get('languages',''),d.get('office_location',''),d.get('preferred_method',''),d.get('linkedin_url',''),d.get('source_url',''),d.get('verified_at',''),d.get('roster_status','Publicly verified'),d.get('notes',''),int(bool(d.get('is_primary'))),int(bool(d.get('is_decision_maker'))),NOW())
        if int(d.get("id") or 0):
            c.execute("""update contacts set name=?,role=?,email=?,phone=?,mobile=?,nmls=?,specialties=?,languages=?,office_location=?,preferred_method=?,linkedin_url=?,source_url=?,verified_at=?,roster_status=?,notes=?,is_primary=?,is_decision_maker=?,updated_at=? where id=? and prospect_id=?""",values+(int(d["id"]),pid))
        else:
            c.execute("""insert into contacts(prospect_id,name,role,email,phone,mobile,nmls,specialties,languages,office_location,preferred_method,linkedin_url,source_url,verified_at,roster_status,notes,is_primary,is_decision_maker,created_at,updated_at) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",(pid,)+values[:-1]+(NOW(),NOW()))
...
def approve_candidate(cid):
    ...
    with db() as c:
        ...
        c.execute("insert into contacts(prospect_id,name,role,email,phone,mobile,nmls,specialties,languages,office_location,preferred_method,linkedin_url,source_url,verified_at,roster_status,notes,is_primary,is_decision_maker,created_at,updated_at) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",(x['prospect_id'],x['name'] or 'Company Contact Desk',x['role'],x['email'],x['phone'],'','','','','','','',x['source_url'],datetime.now().date().isoformat(),'Publicly verified',x['raw_context'],0,0,NOW(),NOW()))
