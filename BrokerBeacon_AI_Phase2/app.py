...
app = Flask(__name__)
BUILD_VERSION = "10.0"
BUILD_NAME = "ASH UNDERWRITER REASONER"
...
function buildUnderwriterAnswer(d,q){
  const answer=$('#underwriterAnswer'),pa=d.plain_answer||{},groups={};
  (d.results||[]).forEach(r=>(groups[r.program]||(groups[r.program]=[])).push(r));
  const verdict=pa.verdict||'I cannot give a reliable answer from the indexed material yet.';
  const explanation=pa.explanation||'The search found related material, but not enough controlling language for a concise conclusion.';
  const confidence=pa.confidence||'Limited';
  const classification=String(pa.classification||'unclear').toLowerCase();
  const label=classification==='yes'?'Likely yes':classification==='no'?'Likely no':classification==='conditional'?'Conditional':'Needs more information';
  ...
  let citationHtml=citations.length?citations.slice(0,6).map(c=>`<span class="answer-source-pill">${esc(c.program||'Official guide')}${c.section?' · '+esc(c.section):''}${c.title?' · '+esc(c.title):''}</span>`).join(''):'';
  answer.innerHTML=`<div class="answer-head"><div><span class="answer-badge">Ash Underwriter</span><div class="answer-title">Direct answer</div></div><span class="guide-chip">${d.results.length} official-source match${d.results.length===1?'':'es'}</span></div>
  <div class="plain-answer-box"><div class="plain-answer-label">Ash's preliminary interpretation <span class="answer-confidence">${esc(confidence)}</span></div><div style="margin-top:8px"><span class="answer-status ${esc(classification)}">${esc(label)}</span></div><div class="plain-answer-verdict">${esc(verdict)}</div><p class="plain-answer-body">${esc(explanation)}</p>${citationHtml?`<div class="answer-citations">${citationHtml}</div>`:''}<div class="answer-detail-grid">${list('Key conditions',conditions)}${list('Information still needed',needed)}${list('Important cautions',cautions)}</div>${pa.basis?`<div class="plain-answer-basis"><b>Basis:</b> ${esc(pa.basis)}</div>`:''}</div>
  ${guideMode==='scenario'&&topics.length?`<div class="scenario-issues">${topics.map(x=>`<span class="scenario-issue">${esc(x)}</span>`).join('')}</div>`:''}
  <div class="answer-caveat"><b>Preliminary guidance only:</b> Confirm the cited guide section’s effective date, AUS findings, UHM and investor overlays, and underwriting interpretation before making a credit or eligibility decision.</div>`;
...
api('/api/version').then(v=>{const el=$('#appVersion');if(el)el.textContent=`VERSION ${v.version} · ${v.build}`}).catch(()=>{});
...
def _plain_english_guideline_answer(query, results):
    ...
    prompt=("You are Ash Underwriter in a wholesale mortgage AE tool. Give a concise, human answer using ONLY the supplied official-guide excerpts. "
            ...
            "Return strict JSON only with these keys: classification, verdict, explanation, conditions, needed_information, cautions, citations, basis, confidence. "
            ...
            "citations must be an array of up to 4 objects with program, section, title, and url copied from the supplied sources. confidence must be High, Moderate, or Limited. "
    ...
    try:
        ...
        if all(parsed.get(k) for k in required):
            ...
            for key_name in ['conditions','needed_information','cautions','citations']:
