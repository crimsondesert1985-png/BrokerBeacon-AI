...
app = Flask(__name__)
BUILD_VERSION = "10.1"
BUILD_NAME = "CONVERSATIONAL ASH UNDERWRITER"
...
function buildUnderwriterAnswer(d,q){
  ...
  ${brokerScript?`<div class="answer-broker-script"><h4>How I’d explain this to a broker</h4><p>${esc(brokerScript)}</p></div>`:''}
...
def _plain_english_guideline_answer(query, results):
    ...
    prompt=("You are Ash Underwriter in a wholesale mortgage AE tool. Give a concise, human answer using ONLY the supplied official-guide excerpts. "
            ...
            "Return strict JSON only with these keys: classification, verdict, explanation, conditions, needed_information, cautions, citations, basis, confidence, broker_script, follow_up_options. "
            ...
            "citations must be an array of up to 4 objects with program, section, title, and url copied from the supplied sources. confidence must be High, Moderate, or Limited. broker_script must be 1-3 natural sentences an AE could say directly to a broker. follow_up_options must be an array of 2-5 short factual choices that would materially refine the answer. "
    ...
    try:
        ...
        if all(parsed.get(k) for k in required):
            ...
            for key_name in ['conditions','needed_information','cautions','citations','follow_up_options']:
