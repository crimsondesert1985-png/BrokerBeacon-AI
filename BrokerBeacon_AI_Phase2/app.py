...
def build_follow_up_preview(c, action, prospect):
    ...
    return {'available':True,'action_id':a['id'],'prospect_id':p['id'],'company':p['company'],
...
def save_follow_up_draft(action_id):
    ...
    with db() as c:
        ...
        cur=c.execute("""insert into outreach(
            prospect_id,channel,subject,body,status,created_at,source_action_id,recommended_send_at,rationale
        ) values(?,?,?,?,?,?,?,?,?)""",(p['id'],channel,subject if channel=='Email' else '',body,'Draft',NOW(),
            action_id,preview['recommended_timing'],preview['rationale']))
