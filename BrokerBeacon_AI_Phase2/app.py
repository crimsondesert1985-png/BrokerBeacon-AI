...
$('#appVersion').textContent='VERSION 12.5 · CALL OUTCOME LOOP';
...
$('#actionForm').onsubmit=async e=>{e.preventDefault();const d=await api('/api/sales-actions',{method:'POST',body:JSON.stringify({prospect_id:+$('#actionPid').value,action_type:$('#actionType').value,outcome:$('#actionOutcome').value,notes:$('#actionNotes').value,objections:$('#actionObjections').value,next_step:$('#actionNext').value,follow_up_date:$('#actionFollow').value,source_view:$('#actionSource').value})});$('#actionDlg').close();e.target.reset();msg(d.followup_created?'Outcome logged · follow-up scheduled':'Outcome logged');await Promise.all([dailyPlan(),dash(),load(),followups()]);if(callPrepProspectId)await openCallPrep(callPrepProspectId)}
...
def sales_action():
    ...
    with db() as c:
        ...
        if not p:return jsonify(error='Prospect not found'),404
        c.execute('insert into sales_actions(prospect_id,action_type,outcome,notes,follow_up_date,created_at,objections,next_step,source_view) values(?,?,?,?,?,?,?,?,?)',(pid,action_type,outcome,notes,follow_up_date,NOW(),objections,next_step,(d.get('source_view') or '').strip()))
    ...
    rescore(pid)
    return jsonify(ok=True,followup_created=followup_created)
