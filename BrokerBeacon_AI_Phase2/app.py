...
app = Flask(__name__)
BUILD_VERSION = "9.0"
BUILD_NAME = "ASH UNDERWRITER ANSWER-FIRST"
...
function buildUnderwriterAnswer(d,q){const answer=$('#underwriterAnswer'),groups={};(d.results||[]).forEach(r=>(groups[r.program]||(groups[r.program]=[])).push(r));let names=Object.keys(groups),top=(d.results||[])[0],topics=scenarioTopics(q);let title=top?`What the indexed guidance supports`:'No sufficiently close indexed match';let summary='';if(top){let first=conciseGuideText(top.excerpt,300);summary=`The strongest match is <b>${esc(top.program)}</b> ${top.section?`<b>${esc(top.section)}</b>, `:''}<b>${esc(top.title)}</b>. ${highlightTerms(first,top.matched_terms||[])}`;}else summary='Ash could not find a close enough indexed section to support an answer. Try the core topic, select one program, or open the official guide links.';
...
answer.innerHTML=`<div class="answer-head"><div><span class="answer-badge">Ash Underwriter research summary</span><div class="answer-title">${title}</div><p class="answer-summary">${summary}</p></div><span class="guide-chip">${d.results.length} source match${d.results.length===1?'':'es'}</span></div>${guideMode==='scenario'&&topics.length?`<div class="scenario-issues">${topics.map(x=>`<span class="scenario-issue">${esc(x)}</span>`).join('')}</div>`:''}${findings?`<div class="answer-findings">${findings}</div>`:''}<div class="answer-caveat"><b>Decision checkpoint:</b> This is a retrieval summary, not an approval or eligibility decision. Confirm the cited source’s effective date, AUS findings, UHM overlays, and underwriting interpretation.</div>`;answer.classList.add('visible')}
