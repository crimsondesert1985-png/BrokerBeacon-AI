...
def install_ember_automation(app, db_path):
    ...
    def scheduled_cycle():
        ...
        with connect() as conn:
            ...
            recent = conn.execute(
                "select created_at from ember_automation_runs where status='Running' order by id desc limit 1"
            ).fetchone()
            if recent:
                try:
                    if datetime.fromisoformat(recent["created_at"]) > datetime.now() - timedelta(minutes=20):
                        return jsonify(status="Skipped", reason="An Ember cycle is already running"), 202
                except ValueError:
                    pass
            started = datetime.now().isoformat(timespec="seconds")
            run_id = int(conn.execute(
                "insert into ember_automation_runs(state,status,created_at) values('NC','Running',?)",
                (started,),
            ).lastrowid)
            conn.commit()
            try:
                result = launch(conn, state="NC", company_limit=4, contact_limit=200)
                import json
                conn.execute(
                    "update ember_automation_runs set status='Completed',detail_json=?,finished_at=? where id=?",
                    (json.dumps(result, default=str), datetime.now().isoformat(timespec="seconds"), run_id),
                )
                conn.commit()
