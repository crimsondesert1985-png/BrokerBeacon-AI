...
function label(el){return (el.getAttribute('aria-label')||el.dataset.help||el.title||el.placeholder||el.textContent||'').replace(/\s+/g,' ').trim()}
...
function eligible(el){return el&&el.closest&&el.closest('button,a,input,select,textarea,[role="button"],[tabindex],.card,.panel,.metric,th,td')}
document.addEventListener('mouseover',e=>{const el=eligible(e.target);if(el)showTooltip(el)});document.addEventListener('mouseout',e=>{const el=eligible(e.target);if(el)hideTooltip(el)});document.addEventListener('focusin',e=>{const el=eligible(e.target);if(el)showTooltip(el)});document.addEventListener('focusout',e=>hideTooltip(e.target));
