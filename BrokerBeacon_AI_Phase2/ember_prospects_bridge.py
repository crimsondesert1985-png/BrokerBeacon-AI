...
from public_search_connector import run_public_search
...
def install_ember_prospects_bridge(app, db_path):
    ...
    def startup_rebuild():
        try:
            with closing(connect()) as conn:
                ...
                existing_urls = int(conn.execute(
                    """select count(*) from public_search_results
                       where source_domain='mortgagematchup.com'
                         and (source_url like '%/Company/%' or source_url like '%/Profile/%')"""
                ).fetchone()[0])
                bootstrap = {"run_id": 0, "accepted": 0, "query_failures": []}
                if existing_urls == 0:
                    try:
                        bootstrap = run_public_search(
                            conn,
                            connector_id=None,
                            state="CA",
                        ...
                        app.logger.exception("EMBER_MATCHUP bootstrap search failed safely")
                        bootstrap = {"run_id": 0, "accepted": 0, "query_failures": [{"error": str(exc)}]}
                ingest = ingest_matchup_results(conn, run_id=int(bootstrap.get("run_id") or 0), state="", limit=1000)
                ...
                app.logger.warning(
                    "EMBER_MATCHUP rebuild removed=%s bootstrap_urls=%s ingested_profiles=%s ingested_companies=%s "
                    "created=%s updated=%s contacts=%s removed_after=%s visible=%s failures=%s",
                    removed_before,
                    bootstrap.get("accepted", 0),
    ...
    def inject_matchup_prospects(response):
        ...
        try:
            ...
            return response
        if "ember-matchup-prospects-v4" in page or "</body>" not in page.lower():
            return response
        script = r'''<style id="ember-matchup-prospects-v4-style">
tr[data-matchup-prospect="1"] td{background:linear-gradient(90deg,#fff,#f7fbff)}.matchup-badge{display:inline-flex;margin-left:7px;padding:3px 7px;border-radius:999px;background:#4f7cff18;color:#315bb4;font-size:9px;font-weight:900}.matchup-sub{display:block;margin-top:4px;color:#637895;font-size:10px}.matchup-message td{text-align:center;padding:32px;color:#637895}
</style><script id="ember-matchup-prospects-v4">(function(){
const esc=v=>String(v??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));const states=new Set('AL AK AZ AR CA CO CT DE FL GA HI ID IL IN IA KS KY LA ME MD MA MI MN MS MO MT NE NV NH NJ NM NY NC ND OH OK OR PA RI SC SD TN TX UT VT VA WA WV WI WY'.split(' '));
function root(){return document.querySelector('#prospects')||document.querySelector('[data-view="prospects"]')||[...document.querySelectorAll('.view,section,main')].find(x=>/prospect/i.test(x.id||'')||/prospect/i.test(x.querySelector('h1,h2')?.textContent||''))}function body(){return root()?.querySelector('#rows,[data-prospect-rows],table tbody')||null}function search(){return root()?.querySelector('#search,[data-prospect-search],input[type="search"]')||null}function state(){return root()?.querySelector('#state,[data-prospect-state],select')||null}function visible(){const r=root();if(!r)return false;const s=getComputedStyle(r);return s.display!=='none'&&s.visibility!=='hidden'}
function row(x){const loc=[x.city,x.state].filter(Boolean).join(', ');return `<tr data-matchup-prospect="1"><td><strong>${esc(x.company_name)}</strong><span class="matchup-badge">MORTGAGE MATCHUP</span><span class="matchup-sub">${Number(x.loan_officer_count||0)} loan officers${x.nmls_id?` · Company NMLS ${esc(x.nmls_id)}`:''}</span></td><td>${x.phone?`<a href="tel:${esc(x.phone)}">${esc(x.phone)}</a>`:''}${x.public_email?`<a href="mailto:${esc(x.public_email)}">${esc(x.public_email)}</a>`:''}<span class="matchup-sub">${esc(x.primary_contact||'Loan-officer profiles attached')}</span></td><td><span class="pill">Mortgage Matchup</span></td><td>${esc(loc)}</td><td><span class="pill">Broker</span></td><td class="score">${Number(x.opportunity_score||90)}</td><td><span class="pill">${esc(x.review_status||'Verify in NMLS')}</span></td><td>${esc(x.pipeline_status||'New')}</td><td><button class="btn smallbtn" data-open-prospect="${Number(x.crm_prospect_id)}">Open</button></td></tr>`}
let seq=0,timer=0;async function load(){const b=body();if(!b||!visible())return;const st=states.has((state()?.value||'').toUpperCase())?state().value.toUpperCase():'';const q=search()?.value||'';const id=++seq;b.innerHTML='<tr class="matchup-message"><td colspan="9">Loading verified Mortgage Matchup prospects…</td></tr>';try{const r=await fetch('/api/ember/main-prospects?limit=5000&state='+encodeURIComponent(st)+'&q='+encodeURIComponent(q),{cache:'no-store',credentials:'same-origin'});if(!r.ok)throw new Error('HTTP '+r.status);const d=await r.json();if(id!==seq)return;b.innerHTML=(d.items||[]).length?d.items.map(row).join(''):'<tr class="matchup-message"><td colspan="9">No verified Mortgage Matchup prospects yet. Ember is rebuilding the directory now.</td></tr>'}catch(e){b.innerHTML='<tr class="matchup-message"><td colspan="9">Mortgage Matchup prospects could not be loaded. Retrying automatically.</td></tr>'}}
function bind(){if(!body())return;const q=search(),s=state();if(q&&!q.dataset.matchupBound){q.dataset.matchupBound='1';q.addEventListener('input',()=>{clearTimeout(timer);timer=setTimeout(load,250)})}if(s&&!s.dataset.matchupBound){s.dataset.matchupBound='1';s.addEventListener('change',load)}body().addEventListener('click',e=>{const btn=e.target.closest('[data-open-prospect]');if(!btn)return;const id=Number(btn.dataset.openProspect);if(typeof window.openProfile==='function')window.openProfile(id)});load()}const observer=new MutationObserver(()=>{if(visible()&&body())bind()});observer.observe(document.documentElement,{subtree:true,childList:true,attributes:true,attributeFilter:['class','style']});if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',bind);else bind();setInterval(load,60000);
})();</script>'''
