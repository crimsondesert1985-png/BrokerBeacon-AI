...
BOOTSTRAP_RECORDS=(
    {
        "url":"https://mortgagematchup.com/Company/EmeryFinancialInc86755",
        "title":"Emery Financial, Inc.",
        "snippet":"NMLS #: 79057 Mortgage Originators (9) Emery Financial, Inc. John Rodrigues NMLS #303266 Adam Heaney NMLS #283076 Alan Godfrey NMLS #910363 Bradford Sarvak NMLS #118685 Arthur Cole NMLS #346048 Contact Information 625 Kings Road Suite 850 Newport Beach, CA 92663 (949) 729-9200 www.emeryfinancial.com",
        "state":"CA","nmls":"79057",
    },
    {
        "url":"https://mortgagematchup.com/Company/UhlerMortgageSolutionsInc34857",
        "title":"Uhler Mortgage Solutions Inc",
        "snippet":"NMLS #: 1825855 Mortgage Originators (20) Uhler Mortgage Solutions Inc Amandeep Kaur NMLS #1451176 Iqbal Kaur NMLS #1456225 Sukhdeep Kaur NMLS #2331164 Kevin Kevin NMLS #2393631 Francisco Molinari NMLS #304806 Contact Information 1731 Hasti Acres Drive Suite 208 #9 Bakersfield, CA 93309 (661) 525-3268 www.umsmtg.com",
        "state":"CA","nmls":"1825855",
    },
)
...
def install_ember_prospects_bridge(app,db_path):
    def connect():
        ...
        conn.execute("pragma foreign_keys=on"); conn.execute("pragma busy_timeout=30000"); return conn

    def seed_bootstrap(conn):
        initialize_search(conn); stamp=NOW()
        run_id=int(conn.execute("""insert into public_search_runs(state,query_count,result_count,accepted_count,rejected_count,status,created_at,started_at,finished_at) values('CA',1,?,?,0,'Completed',?,?,?)""",(len(BOOTSTRAP_RECORDS),len(BOOTSTRAP_RECORDS),stamp,stamp,stamp)).lastrowid)
        inserted=0
        for rank,record in enumerate(BOOTSTRAP_RECORDS,1):
            cur=conn.execute("""insert or ignore into public_search_results(run_id,query_text,result_rank,title,snippet,source_url,source_domain,candidate_type,company_name,person_name,state,nmls_id,phone,public_email,provider_name,review_status,created_at) values(?,?,?,?,?,?,?,?,?,'',?,?, '', '',?,'Pending review',?)""",
                (run_id,"Verified Mortgage Matchup indexed company record",rank,record["title"],record["snippet"],record["url"],"mortgagematchup.com","Company",record["title"],record["state"],record["nmls"],"Mortgage Matchup public index",stamp))
            inserted+=int(cur.rowcount>0)
        conn.commit(); return run_id,inserted

    def startup_rebuild():
        try:
            with closing(connect()) as conn:
                removed_before=purge_invalid_ember_prospects(conn)
                run_id,inserted=seed_bootstrap(conn)
                ingest=ingest_matchup_results(conn,run_id=run_id,state="CA",limit=1000)
                ...
                rows=conn.execute("""select distinct p.company,p.nmls from prospects p join autonomous_prospect_links l on l.prospect_id=p.id join warehouse_source_records wr on wr.entity_type='company' and wr.entity_id=l.warehouse_company_id join warehouse_sources s on s.id=wr.source_id where s.name=? order by p.company""",(MATCHUP_SOURCE,)).fetchall()
                app.logger.warning("EMBER_MATCHUP rebuild removed=%s seeded=%s urls=%s indexed_fallbacks=%s companies_created=%s companies_updated=%s officers_created=%s officers_updated=%s prospects_created=%s prospects_updated=%s contacts_created=%s removed_after=%s visible=%s samples=%s failures=%s",
                    removed_before,inserted,ingest.get("urls_found",0),ingest.get("indexed_fallbacks",0),ingest.get("companies_created",0),ingest.get("companies_updated",0),ingest.get("officers_created",0),ingest.get("officers_updated",0),promotion.get("prospects_created",0),promotion.get("prospects_updated",0),promotion.get("contacts_created",0),removed_after,len(rows),[(r[0],r[1]) for r in rows[:5]],len(ingest.get("failures",[])))
                for failure in ingest.get("failures",[])[:10]: app.logger.warning("EMBER_MATCHUP ingest failure url=%s error=%s",failure.get("url",""),failure.get("error",""))
        except Exception:
    ...
    def ember_main_prospects():
        ...
        with closing(connect()) as conn:
            rows=conn.execute("""select distinct p.* from prospects p join autonomous_prospect_links l on l.prospect_id=p.id join warehouse_source_records wr on wr.entity_type='company' and wr.entity_id=l.warehouse_company_id join warehouse_sources s on s.id=wr.source_id where s.name=? and (?='' or upper(trim(coalesce(p.state,'')))=?) and (?='' or lower(coalesce(p.company,'')) like ? or lower(coalesce(p.city,'')) like ? or lower(coalesce(p.nmls,'')) like ?) order by coalesce(p.updated_at,p.created_at,'') desc,p.id desc limit ?""",(MATCHUP_SOURCE,state,state,query,like,like,like,limit)).fetchall()
            items=[]
            for p in rows:
                ...
                items.append({"id":p["id"],"crm_prospect_id":p["id"],"company_name":p["company"] or "","nmls_id":p["nmls"] or "","source_url":p["source_url"] or p["website"] or "https://mortgagematchup.com","phone":p["phone"] or (primary["phone"] if primary else "") or "","public_email":p["email"] or (primary["email"] if primary else "") or "","city":p["city"] or "","state":p["state"] or "","review_status":p["verification_status"] or "Verify in NMLS","pipeline_status":p["status"] or "New","opportunity_score":int(p["score"] or 90),"loan_officer_count":len(contacts),"primary_contact":primary["name"] if primary else "","source":"Mortgage Matchup","record_type":"crm"})
