...
def launch(conn:sqlite3.Connection,*,state:str='',company_limit:int=6,contact_limit:int=250)->dict:
 ...
 for rank,row in enumerate(_seed_rows(conn,state,after_id,company_limit*8),start=1):
  ...
  conn.execute("""insert or ignore into public_search_results(run_id,query_text,result_rank,title,snippet,source_url,source_domain,candidate_type,company_name,city,state,nmls_id,review_status,created_at) values(?,?,?,?,?,?,?,?,?,?,?,?, 'Pending review',?)""",(run_id,'existing-index-seed',rank,company,'Queued by Ember from Broker Index.',url,domain,'Company',company,city,state,nmls,now))
