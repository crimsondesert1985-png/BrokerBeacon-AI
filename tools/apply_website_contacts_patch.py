...
def main():
    ...
    if "html_lib.unescape" in text:
        ...
        changed = True
        print("fixed html_lib")
    old_paths = "paths=['','/team','/about','/about-us','/our-team','/loan-officers','/meet-the-team']"
    new_paths = "paths=['','/team','/about','/about-us','/our-team','/loan-officers','/meet-the-team','/contact','/contact-us','/staff','/people']"
    if old_paths in text:
        text = text.replace(old_paths, new_paths, 1)
        changed = True
        print("expanded paths")
    if "imported {imported} contact(s), staged" not in text:
        needle = 'rows=[dict(x) for x in c.execute("select * from contact_candidates where prospect_id=? and status=\'Pending\' order by id desc",(pid,))]\n    return jsonify(message=f"Checked {len(checked)} official company page(s) and staged {len(rows)} unapproved discovery item(s).",candidates=rows)'
        insert = (
            "imported=0\n"
        ...
        changed = True
        print("auto-promote added")
    old_js = "try{let r=await api('/api/prospects/'+current.id+'/refresh-contacts',{method:'POST'});renderCandidates(r.candidates||[]);msg(r.message)}"
    new_js = "try{let r=await api('/api/prospects/'+current.id+'/refresh-contacts',{method:'POST'});if(r.contacts){current.contacts=r.contacts;renderContacts(r.contacts)}renderCandidates(r.candidates||[]);msg(r.message||'Website review complete')}"
    if old_js in text and "if(r.contacts)" not in text:
        text = text.replace(old_js, new_js, 1)
        changed = True
        print("ui refresh wired")
    old_profile = "async function profile(id){let p=await api('/api/prospects/'+id);current=p;updateAshContext();renderContacts(p.contacts||[]);clearContactForm();loadCandidates();"
    new_profile = old_profile + "(function(){const named=(p.contacts||[]).filter(c=>c.name&&c.name!=='Company Contact Desk');if(!named.length)setTimeout(()=>{if(current&&current.id===p.id&&$('#refreshRoster'))$('#refreshRoster').click()},500)})();"
    if old_profile in text and "if(!named.length)" not in text:
        text = text.replace(old_profile, new_profile, 1)
        changed = True
        print("auto review on empty roster")
    if not changed:
        print("no changes")
        return 0
    APP.write_text(text)
